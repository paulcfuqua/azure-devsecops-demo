...
describe("the telemetry initializer", () => {
  const initializer = createTelemetryInitializer("launch-ops");

  it("stamps the cloud role V7.3 asserts on", () => {
    const envelope: TelemetryEnvelope = {};
    initializer(envelope);
    expect(envelope.tags?.["ai.cloud.role"]).toBe("launch-ops");
  });
...
describe("initBrowserTelemetry", () => {
  it("does nothing and loads no SDK when no connection string is configured", async () => {
    const onError = vi.fn();
    await expect(
      initBrowserTelemetry({ cloudRole: "launch-ops", env: {}, win: {}, onError }),
    ).resolves.toBe(false);
