...
function Test-GhasFeature {
    ...
    $alertsEnabled = ("$alerts" -match '(?m)^HTTP/[\d.]+\s+204')
    $observed.Add("vulnerability-alerts=$(if ($alertsEnabled) { '204' } else { 'not 204' })")
    if (-not $alertsEnabled) { $problem.Add('the vulnerability-alerts endpoint did not answer 204 - Dependabot alerts are off') }
