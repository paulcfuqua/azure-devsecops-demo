...
| **1** | **Copilot service** — Ask tab over Direct Line | 🔴 **transport healthy, agent makes zero tool calls** | **Now measured rather than inferred (F183 → F184).** The eval could never run: it read the Direct Line secret as an identity holding no Key Vault role, reported the secret ABSENT rather than UNOBSERVABLE, and suppressed its own artifact — so V8.2/V8.4/V8.5 skipped and L8 stayed green over a silent agent, for the life of the project. Both halves fixed and merged; **`layer-08-agent-eval` uploaded for the first time ever**, reporting **0/10 (bar 9)**, p95 4.0s, **0 tool calls and 0 cards across all ten questions**, no transport errors. Direct Line connects and a conversation allocates; the agent routes to its built-in `Escalate` and fallback topics. Reproduced in a browser. **Zero invocations rules OUT a stale MCP FQDN** — a bad endpoint yields failing tool calls, not absent ones — and points at the solution's topics and tool components. Next investigation, with evidence in hand |
