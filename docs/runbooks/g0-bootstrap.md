...
| `MLS_L10_DEPENDABOT_ALERTS` | ~~the three seeded Dependabot alert numbers~~ | **RETIRED 2026-09-07 — do not set.** It named the three seeded pins for the old V10.2. The operations model reads the whole alert surface and verifies every closure in a declared window, so there is no pre-named list to keep current. Service levels are declared in `.github/self-heal-policy.json` instead | **never** |
