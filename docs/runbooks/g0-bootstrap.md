...
| `MLS_L10_RESEED_MERGED_AT` | ~~UTC time the re-seed PR merged~~ | **RETIRED — do not set, and do not re-seed to produce a value.** It dated V10.1/V10.2's 24-hour window from a re-seed of `apps/vuln-lab`; the sponsor-approved design of 2026-09-05 (PR #237) retires that plant, and F192 already derives the window from the heal PR's auto-merge arming time instead | **never** |
| `MLS_L10_DEPENDABOT_ALERTS` | the three seeded Dependabot alert numbers, comma- or space-separated. **Set on the `verify` environment** — that is the environment the L10 verify job declares, and a `demo` variable is invisible to it | V10.2. `self-heal.yml` appends the alert its own run picked, so a fresh alert is covered before anyone updates this. **Dies with the plant**: the 2026-09-05 design replaces a pre-named alert list with a per-healed-finding trail | L10, until the migration |
