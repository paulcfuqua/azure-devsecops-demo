...
**Outcome: landed as planned, with one primitive swapped for a more robust one and one extra defense layer added beyond the brief.** The brief's Step 2 suggested filtering `gh pr list`'s `headRepositoryOwner.login` to the base repo's owner; shipped instead with `gh api repos/${REPO}/git/matching-refs/heads/self-heal/`, which is rooted at the base repository's own ref namespace — a fork's branch lives in the fork's own namespace and can never appear there, so this sidesteps the fork question rather than filtering for it, and (verified empirically against a real public repo, `cli/cli`, including the zero-match case: HTTP 200 with an empty array, never 404 — so a clean repo with no self-heal branches open yet still heals on its very first run) needs only `contents: read`, a permission the `select` job already carries, where `gh pr list` would need `pull-requests: read` that job does not have. The code-scanning alerts endpoint gained `ref=refs/heads/<default_branch>`, with the default branch resolved dynamically via `repos/${REPO}` rather than hardcoded `main` (matching the pattern the `autofix` job's own branch-creation step already used), and the alert-selection loop re-checks `.most_recent_instance.ref` against that same target directly from the listing JSON rather than trusting the query parameter alone. A second, independent assertion of `.most_recent_instance.ref` was added as a new first step of the `autofix` job, because the explicit-number path (`workflow_dispatch` / `repository_dispatch`) bypasses the `select` job's listing — and its `ref=` filter — entirely; without this, the fix would protect only the scheduled-poll path and leave a manually-dispatched alert number unchecked, hard-failing that job with an `::error` if the alert's most recent instance is off the default branch. `verification/tests/self-heal-selection.Tests.ps1` (5 tests) is a workflow-shape regression guard, the available mechanism per CLAUDE.md since this file has no execution test harness: it asserts the vulnerable `gh pr list --state open --json headRefName` invocation is gone, that `git/matching-refs/heads/self-heal/` is present, that the code-scanning endpoint carries `ref=`, and that `most_recent_instance.ref` is checked at least twice — confirmed with teeth by running it against the pre-fix file (4 of 5 fail) before running it against the fixed one (5/5 pass). `actionlint` is clean on both the original and the `ref-check` step addition. Pester `scripts,infra,data,verification,compliance` moved 633 -> 638 (the 5 new tests); root `npm test` unchanged at 868, exit 0 (control-tower 98, launch-ops 33, data-api 295, mcp-tools 287, directline-token 12, cost-ingest 84, spec-renderer 59) — no JS touched. Register: F14 closed. F14 maps to no 800-171 control (availability), so unlike a controlled finding there is no `compliance/assessment/*.json` to update; F14 and F15 are the only two findings in this register without one, and F14's `Status` field in `compliance/findings/2026-08-26-prepublication-review.md#f14` is updated directly to `CLOSED` — that entry and this note are its only closure record.
...
**Outcome: landed as planned, with the implementation mechanism changed by a real defect the brief flagged as a risk and asked to be checked for.** The container-name fix was a one-line swap (`costexports` -> `cost-exports`, matching `infra/bicep/platform/main.bicep` and V6.3's default), but the grant was not: `az costmanagement export create`/`update` has no `--identity-type` flag at all, and the costmanagement CLI extension pins API version 2020-06-01, which — confirmed against `github.com/Azure/azure-cli/issues/32912` and Microsoft's own Exports tutorial — hard-requires the destination storage account's shared keys to be enabled; it would 400 outright against this account's `allowSharedKeyAccess: false` before a role assignment ever became relevant. The fix replaces the CLI call with an `az rest` PUT against the Exports REST API directly (api-version 2023-08-01, which supports both RBAC-only storage and an `identity: { type: SystemAssigned }` request in the body); the response returns that identity's `principalId` synchronously, and the workflow grants it `Storage Blob Data Contributor` — scoped to the `cost-exports` container, not the whole account — via an idempotent (check-then-create) `az role assignment create` keyed on the `principalId`, never a resourceId or clientId (Task 12's own caution, repeated because it is exactly as easy to get wrong a second time). `scripts/bootstrap/03-budget.ps1` gained `Forecasted` notifications at 50%/80% alongside the existing `Actual` ones at 50/80/100% — additive, verified by a test asserting both sets coexist (3 Actual + 2 Forecasted), not a replacement, since Actual remains the ground truth once its 8-24h lag clears. TDD: the brief's exact test failed (3 assertions, not 1 — the new test plus two the implementer added alongside it) against the pre-fix script, confirmed by stashing only the implementation change and re-running; passed 11/11 after restoring it. `actionlint`'s own workflow-syntax/expression checks pass clean on the file (confirmed via `-shellcheck=` to isolate them), and the new script independently shellchecks clean under actionlint's exact flags (both as a standalone file and piped via stdin) and inside a minimal single-step workflow run through the real binary with shellcheck enabled — but the literal `actionlint <file>` command hangs indefinitely on this Windows-arm64 dev host once this specific file crosses some cumulative shellcheck-subprocess threshold (reproduced 4 times, always CPU-idle while blocked, never content-dependent); CI's `lint-ci.yml` runs the pinned `linux_amd64` actionlint release plus ubuntu's native shellcheck, a different binary/platform pair, and is the authoritative gate. Pester `scripts,infra,data,verification,compliance` moved 638 -> 641 (3 new); root `npm test` unchanged at 868, exit 0 (control-tower 98, launch-ops 33, data-api 295, mcp-tools 287, directline-token 12, cost-ingest 84, spec-renderer 59) — no JS touched. Register: F15 closed — maps to no 800-171 control, so its `Status` field in `compliance/findings/2026-08-26-prepublication-review.md#f15` is updated directly to `CLOSED` with a closing paragraph, the same pattern F14 used. F13's three assessment files (`3.1.1.json`, `3.1.2.json`, `3.1.5.json`) were updated to six-of-seven grants landed; F13 itself stays `GAP` — F19 (cost-ingest's grant) is the sole remaining contributor.
