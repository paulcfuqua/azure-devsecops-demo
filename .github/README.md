...
| `MLS_COMPLIANCE_CLIENT_ID` | variable | L7 (`layer-07-apps.yml`) | Entra **application (client) ID** Easy Auth validates compliance-board sign-ins against. **Not a secret** — an OAuth client ID is a public identifier, the same trust class as the tenant/subscription IDs above, and no client secret is configured for the provider. Registration `mls-compliance-demo-app`, created at L3. **Optional override (F36):** leave it unset and `layer-07-apps.yml` resolves the ID from Entra by that registration's manifest display name; set it to bring your own registration |
| `MLS_LAUNCH_OPS_CLIENT_ID` | variable | L7 (`layer-07-apps.yml`) | Same, for `launch-ops` (registration `mls-launch-ops-demo-app`). **Optional override** — F25: launch-ops proxies `/api/` to `data-api`, whose identity holds Security Reader at subscription scope, so an app with no resolvable client ID deploys internal-only rather than anonymously reachable |
| `MLS_CONTROL_TOWER_CLIENT_ID` | variable | L7 (`layer-07-apps.yml`) | Same, for `control-tower` (registration `mls-control-tower-demo-app`). **Optional override** — F25 was demonstrated against this app: `GET /api/feeds/secure-score` returned live Defender posture to anonymous callers |
