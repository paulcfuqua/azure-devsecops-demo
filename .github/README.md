...
| `MLS_COMPLIANCE_CLIENT_ID` | variable | L7 (`layer-07-apps.yml`) | Entra **application (client) ID** Easy Auth validates compliance-board sign-ins against. **Not a secret** — an OAuth client ID is a public identifier, the same trust class as the tenant/subscription IDs above, and no client secret is configured for the provider. Registration `mls-compliance-demo-app`, created at L3. **Required:** `layer-07-apps.yml` refuses to deploy without it (F25/F26 — an unset GitHub variable expands to the EMPTY STRING, so the template's `'unset'` default was never reached and nothing failed) |
| `MLS_LAUNCH_OPS_CLIENT_ID` | variable | L7 (`layer-07-apps.yml`) | Same, for `launch-ops` (registration `mls-launch-ops-demo-app`). **Required** — F25: launch-ops proxies `/api/` to `data-api`, whose identity holds Security Reader at subscription scope, so it must not be anonymously reachable |
| `MLS_CONTROL_TOWER_CLIENT_ID` | variable | L7 (`layer-07-apps.yml`) | Same, for `control-tower` (registration `mls-control-tower-demo-app`). **Required** — F25 was demonstrated against this app: `GET /api/feeds/secure-score` returned live Defender posture to anonymous callers |
